...
{
    public partial class ModuleDesignProgressControl : CommonUserControl
    {
        private static string id = "8F2A6C15-4D9B-4E7A-B3C6-1A5E8D2F9C40";

        public ModuleDesignProgressControl()
        {
            if (!chkPrivilege(id))
            {
        ...
        {
            var rep = new ProjectProgressController().GetModuleDesignProgressList(projectNo);
            if (!string.IsNullOrEmpty(rep.ErrorMessage))
            {
                MessageBox.Show(rep.ErrorMessage);
                return;
            }
            FillGrid(rep.resultList ?? new List<模組設計進度表>());
        }

        private void FillGrid(List<模組設計進度表> list)
        {
