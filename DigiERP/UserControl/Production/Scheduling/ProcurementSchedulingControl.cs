...
{
    public partial class ProcurementSchedulingControl : CommonUserControl
    {
        private static string id = "7C4E9A16-3F8D-4B62-A9C1-5E2D8F6A3B71";
        private const double 每週應計工時 = 40;

        public ProcurementSchedulingControl()
        {
            if (!chkPrivilege(id))
            {
        ...
        internal void LoadData(DateTime 基準日以前, DateTime 第一週, DateTime 第二週, DateTime 第三週, DateTime 第四週)
        {
            ...
            var rep = new ProjectProgressController().GetProcurementScheduleList(基準日以前, 第一週, 第二週, 第三週, 第四週);
            if (!string.IsNullOrEmpty(rep.ErrorMessage))
            {
                MessageBox.Show(rep.ErrorMessage);
                return;
            }
            var list = rep.resultList ?? new List<採購週排程表>();
            FillGrid(list);
            FillSummary(list);
            PositionDateHeaders();
        }

        private void FillGrid(List<採購週排程表> list)
        {
        ...
        // ── 每週派案工時(S0-S4,S未排)＝各欄位有預計到貨日之筆數(此處為到貨日而非工時，故以筆數計)；
        // 每周應計工時固定40；負荷率＝S/40 ─────────────────────────────
        private void FillSummary(List<採購週排程表> list)
        {
            ...
            rowHours.Cells[6].Value = sUnscheduled;
            ...
            rowLoad.Cells[6].Value = (sUnscheduled / 每週應計工時).ToString("P0");
