        ...
        public string saveDesignAudit(設計派案 header, List<設計審查明細> details)
        {
            using (var conn = new SqlConnection(IRepository<string>.ConnStr))
            {
                ...
                using (var tran = conn.BeginTransaction())
                {
                    ...
                    conn.Execute(@"
UPDATE 設計派案
   SET 審查日期=@審查日期, 審查人員=@審查人員, 審圖通過=@審圖通過, 已發行=@已發行, 發行人員=@發行人員, 圖檔發行日=@圖檔發行日
 WHERE 設計識別碼=@設計識別碼",
                        new { header.審查日期, header.審查人員, header.審圖通過, header.已發行, header.設計識別碼
                        , 發行人員 = (((bool)header.已發行)?header.發行人員:"")
                        , 圖檔發行日 = (((bool)header.已發行)?header.圖檔發行日 : "")}, tran);
...
VALUES
    (@BOM編號, @專案序號, @模組編碼, @模組名稱, @設計人員, @製圖檔名, @圖檔發行日, @發行人員, @審查清單編號, @建檔, @建檔日)",
        ...
        // ── 圖面發行總覽：列出尚未指定用途的專案模組用料清單，帶出各筆BOM明細 ──
        public List<專案用料總覽> getModuleMaterialOverviewList()
        {
            using (var conn = new SqlConnection(IRepository<string>.ConnStr))
            {
                conn.Open();
                return conn.Query<專案用料總覽>(@"
SELECT
    dbo_專案模組用料清單.BOM編號,
    dbo_專案模組用料清單.專案序號,
    dbo_專案模組用料清單.模組編碼,
...
FROM
    專案模組用料清單 dbo_專案模組用料清單
    LEFT JOIN 專案模組BOM明細 dbo_專案模組BOM明細 ON dbo_專案模組用料清單.BOM編號 = dbo_專案模組BOM明細.BOM編號
WHERE
    (((dbo_專案模組用料清單.用途) IS NULL))").ToList();
            }
        }

        // ── 組裝派案及領料作業：列出全部專案模組用料清單表頭(不篩選用途) ──────
        public List<專案模組用料清單> getModuleMaterialFullList()
        {
            using (var conn = new SqlConnection(IRepository<string>.ConnStr))
            {
                conn.Open();
                return conn.Query<專案模組用料清單>(@"
SELECT
    dbo_專案模組用料清單.BOM編號,
    dbo_專案模組用料清單.專案序號,
    dbo_專案模組用料清單.模組編碼,
    ...
    dbo_專案模組用料清單.用途
FROM
   專案模組用料清單 dbo_專案模組用料清單").ToList();
            }
        }

        // ── 組裝派案及領料作業：批次儲存編修後的組裝資訊，僅更新表頭欄位，
        //    不觸碰專案模組BOM明細(避免誤刪領料明細) ─────────────────────
        public void saveModuleMaterialHeaderBatch(List<專案模組用料清單> list, string operatorName)
        {
            using (var conn = new SqlConnection(IRepository<string>.ConnStr))
            {
                conn.Open();
                using (var tran = conn.BeginTransaction())
                {
                    foreach (var x in list ?? new List<專案模組用料清單>())
                    {
                        conn.Execute(@"
UPDATE 專案模組用料清單
...
VALUES (@BOM編號, @球號, @零件號碼, @品名, @描述, @數量, @最後修改日期, @上一階品號, @不需備料, @備註, @勾選)", d, tran);
        ...
        // ── 材料領用清單新增明細：可領用零件清單，依專案序號+模組編碼篩選，
        //    僅列出零件分類為市購品/庫存品(已採購入庫，可供領用)的零件 ──────
        public List<可領用零件清單> getPurchasablePartListForBom(string projectNo, string moduleCode)
        {
            using (var conn = new SqlConnection(IRepository<string>.ConnStr))
            {
                conn.Open();
                return conn.Query<可領用零件清單>(@"
SELECT
    dbo_A零件組合單.產品編號,
    dbo_採購計畫.零件號碼,
    dbo_採購計畫.品名,
    ...
    dbo_採購計畫.BOM編號
FROM 採購計畫 dbo_採購計畫
LEFT JOIN A零件組合單 dbo_A零件組合單 ON dbo_採購計畫.採購識別碼 = dbo_A零件組合單.採購識別碼
WHERE dbo_採購計畫.專案序號=@專案序號
  AND dbo_採購計畫.模組編碼=@模組編碼
  AND dbo_採購計畫.零件分類 IN ('市購品', '庫存品')",
                    new { 專案序號 = projectNo, 模組編碼 = moduleCode }).ToList();
            }
        }
